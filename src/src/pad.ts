...
function getDiffButtons(buttons: readonly GamepadButton[], old: GamepadButton[]): { down: Keys[], up: Keys[] } {
    const down: Keys[] = []
    const up: Keys[] = []
    buttons.forEach((e, i) => {
        if (e.pressed !== old[i]?.pressed) {
            if (e.pressed) {
                down.push(keyMap[i])
            } else {
                up.push(keyMap[i])
...
function getDiffSticks(sticks: readonly number[], old: number[]): { r?: StickValue, l?: StickValue } {
...
function getRad(axes: readonly number[]) {
    return [((axes[1] || axes[0]) ? true : 0) && Math.atan2(axes[1], axes[0]), ((axes[3] || axes[2]) ? true : 0) && Math.atan2(axes[3], axes[2])]
}
